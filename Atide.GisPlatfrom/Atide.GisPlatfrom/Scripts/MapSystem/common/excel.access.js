...
function to_json(workbook) {
    //require(["atide/gis/config/system-config"], function (SystemConfig) {
        ...
        var roa = X.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[g_sheetIndex - 1]], { header: g_fieldName, range: g_rangeTop });
        if (roa.length > 0) {
            roa.splice(roa.length - g_rangeBottom, g_rangeBottom);
        }
        else {
            roa = {};
        }
